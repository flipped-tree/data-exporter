...
public class JsonUtil {
     ...
     * @param obj
     */
    public static void obj2File(String fileName,
                                Object obj) {
        if (obj == null) {
            return;
        }
     ...
     * @return 自定义对象
     */
    public static <T> T string2ObjFieldLowerCamelCase(String str,
                                                      Class<T> clazz) {
        if (StringUtils.isEmpty(str) || clazz == null) {
            return null;
        }
        try {
            return clazz.equals(String.class) ? (T) str : OBJECT_MAPPER_SNAKE_CASE.readValue(str, clazz);
        } catch (Exception e) {
     ...
     * @return 自定义对象
     */
    public static <T> List<T> string2ListFieldLowerCamelCase(String str,
                                                             TypeReference<List<T>> typeReference) {
        if (StringUtils.isEmpty(str) || typeReference == null) {
            return null;
        }
        try {
            return OBJECT_MAPPER_SNAKE_CASE.readValue(str, typeReference);
        } catch (Exception e) {
    ...
    public static <T> String obj2StringPretty(T obj) {
        ...
        try {
            return obj instanceof String ? (String) obj : OBJECT_MAPPER.writerWithDefaultPrettyPrinter().writeValueAsString(
                    obj);
        } catch (JsonProcessingException e) {
            log.error("Parse Object to String error ", e);
            return null;
